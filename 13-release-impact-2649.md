...
他 6 ハーネスの利用者はこの環境変数を出荷設定として持たないため、
シェルや CI で自分で設定しない限り、常にハードコード既定（新 `classic`）に到達する。
...
DocumentKB は**ローカル完結**である。`fetch(` および `http(s)://` の出現は
`aidlc-knowledge.ts` 内に **0 件**で、抽出は `spawnSync` でローカル実行ファイル
（既定は PATH 上の `pdftotext`）を呼ぶだけである。外部 API に文書が送られることはない。
...
| **copilot** | ✗ | **2.6.12**: 進行中ワークフローがあれば**新しい会話を開始**する。**2.6.48**: 再コピーでプレースホルダ持ちペルソナを差し替える |
| **opencode** | ✗ | **2.6.48**: 再コピーでプレースホルダ持ちペルソナを差し替える |
