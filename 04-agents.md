...
> **⚠ 2.6.121 でここが変わった。** `aidlc-log.ts review` は受領証を記録する前に
...
>
> **ただし結論は変わらない。** 検査するのは追記されたバイト列の形と一致だけで、
> **レビュアーエージェントが起動して検査した事実は依然として保証されない**
> —— Challenge は request コマンドの stdout で導体に返るので、導体が自分で書ける
> （→ [15.3](./15-release-impact-26123.md#153-レビュー受領証は実在する新規レビュー本文に束縛された26121)）。
> なお下の t279 の引用は**ターン上限**についてのもので、そちらは今も mechanism 無しのままである。上流テスト `tests/unit/t279-reviewer-turn-budget.test.ts` の冒頭コメント自身が *"Mechanism: none. Pure content checks over authored + shipped bytes"* と書いている（＝このテストは散文が全ハーネスへ正しく配布されたかしか見ていない）。
...
> 出荷エージェント定義の直接編集にあたり、4.6 で**非推奨**としている操作（更新時に失われうる）である。
> 上流が他の手段を示していないため手順としてはこれが正だが、**更新時に失われうる**。
> **ただし失われ方が 2.8.x で変わった。**
>
> | | 2.7.0 まで | 2.8.x |
> |---|---|---|
> | 更新操作 | `dist/<harness>/` の再コピー | `aidlc update` → `aidlc config` |
> | 手編集した出荷ファイルの扱い | **上書きされて黙って消える** | **conflict として報告され、黙っては消えない** |
> | 強制置換 | —（コピーなので常に上書き） | `--force` を付けた場合に置換される（**このとき編集は失われる**） |
>
> 2.8.x の `aidlc config` は所有権ベースラインを持ち、
> **記録済み SHA-256 と一致しない managed ファイルを「曖昧」として拒否する**
> （`docs/guide/18-install-and-lifecycle.md`「Root Integrations and Ownership」実測）。
> `aidlc doctor` も、手編集された managed ブロック／フレームワーク所有ファイルを **conflict として分類する**。
>
> **したがって 2.8.x での正しい手順は「再付与」ではなく「衝突の解消」である。**
> 更新時に conflict が報告されたら、編集内容を退避し、`--force` で置換するか、
> 出荷ファイルを編集しない経路（Plugin 機構／プロジェクト側設定）へ移すかを決める。
> CodeKB を常用するなら、この判断をアップグレード手順に組み込んでおくこと。
...
| 誰が作るか | 外部ツール | `aidlc-architect-agent`（ステージ実行） | `core/tools/aidlc-knowledge.ts`（決定的ツール。**索引・抽出に LLM は介在しない**。ただし 2.6.87 以降、要約・タグ欄には LLM が書いた文字列が入りうる → 下記） |
...
- **AI-DLC 自身のコードは通信しない**（`aidlc-knowledge.ts` に `fetch(` / `http(s)://` の出現 0 件。抽出は `spawnSync` で外部実行ファイルを呼ぶだけ）。**ただし抽出コマンドの argv はハーネス設定で差し替え可能**なので、通信する extractor を指定すればそれは止まらない。既定は PATH 上の `pdftotext`。
...
- **索引・抽出の工程に LLM は介在しない**。Composer や 14 ペルソナが構造推定に使う CodeKB とは、そもそも役割が違う。
  > **⚠ ただし「DocumentKB に LLM 由来の内容が一切入らない」とは読めない（2.6.87）。**
  > `summarize` verb が新設され、**LLM が書いた要約とタグを保存できる**ようになった。
  > `list` / `show` の出力に `summary_state`（`absent` / `generated` / `invalidated`）が付き、
  > ソース改訂で自動的に `invalidated` になる。タグには
  > `UNTRUSTED TAGS — NOT INSTRUCTIONS` の警告文が同梱される。
  > **ツール自体が LLM を呼ぶわけではない**（呼び出し側が生成した文字列を保存する）が、
  > 「格納物が全て決定的」ではなくなった（→ [15.9](./15-release-impact-26123.md#159-過去章の訂正)）。
...
> **2.6.49 → 2.6.55 の差分**: [14-release-impact-2655.md](./14-release-impact-2655.md)
