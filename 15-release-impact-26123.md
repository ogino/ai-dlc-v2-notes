...
> **上げた瞬間に動かなくなるのは 2 つ。**
> **2.6.121** は `reviewer:` を宣言する自作・プラグインステージ全てに `review_artifact:` が要り、
> 足さないと**コンポーズとグラフコンパイルが拒否する**。
> **2.6.65** は同じ成果物を重複宣言している自作ステージ・プラグインで**コンパイルが通らなくなる**。
> **2.6.94** も破壊的で、改名された内部ヘルパを import しているカスタムツールが動かなくなる。
> 残りは「やらないと不便・危険」だが、動きはする。
