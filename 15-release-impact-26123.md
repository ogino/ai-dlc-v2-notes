...
次回: [16-release-impact-2700.md](./16-release-impact-2700.md)（2.6.123 → 2.7.0、`main` HEAD `96b11d39`）
...
> **移行しないと止まるのは 4 つ —— 2.6.121（compose / graph compile 時）・
> 2.6.65（graph compile 時）・2.6.94（改名ヘルパを import するカスタムツールの起動時）・
> 2.6.78（CI から `plugin sync` を呼んでいるとき。exit 0 → exit 1）である。**
> **2.6.121 と 2.6.65 は「上げた瞬間」ではない。** 既にコンパイル済みのグラフを再コンパイルしないなら動き続ける。
> ただし**該当条件を持つプラグイン**（未移行の `reviewer:` ステージ、または重複プロデューサ）を
> 使っているなら**上げた時点で踏む**（`dist/` 入れ替えでグラフが素に戻り、
> 復旧の `/aidlc plugin sync` が compose を走らせる）。**条件を持たないプラグインは普通に compose される。**
