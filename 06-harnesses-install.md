...
> 2.7.1 以前の `dist/` からコピーして手編集したファイル（CodeKB 用に `tools` を足した Composer 定義など）は
> **2.8.x の所有権ベースラインに登録が無く、「曖昧な未登録内容」として拒否される。**
> **`--force` でも置換できない。** `aidlc config` がそこで止まる。
> 手順は **編集内容を退避 → そのファイルを移動または削除 → `aidlc config` に作り直させる**
> （詳細と対処の分岐は [4.6 カスタマイズの正しい場所](./04-agents.md#46-カスタマイズの正しい場所)）。
>
