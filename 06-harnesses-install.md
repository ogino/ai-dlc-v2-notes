...
### 実行環境の前提: 単一ローカルファイルシステム（2.6.51 以降）
...
**したがって、リポジトリをネットワーク共有や同期フォルダ（クラウドドライブの同期対象など）に置いている利用者は、
2.6.51 で新規に影響を受ける。** 2.6.51 は `emit()` の fail-open を削除したため、
マーカーの公開に失敗すると**作業ディレクティブを一切出さずに打ち切るハードエラー**になる
（それ以前は Copilot 以外なら公開失敗を握り潰してディレクティブを配信していた）。上流の文言:
...
> NFS / SMB / FUSE / オブジェクト同期フォルダは上流が unsupported と明記している。
> → [6.2 実行環境の前提](#実行環境の前提-単一ローカルファイルシステム2651-以降)
