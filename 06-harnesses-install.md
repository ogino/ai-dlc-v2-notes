...
> 本節の表は **2.6.55 までの手順**である。2.6.123 へ上げるときは、下の 8 点を追加で行うこと
...
> | 8 | **2.6.65** | 同じ成果物を `produces` / `optional_produces` に重複宣言していると**グラフのコンパイルが通らなくなる**。片方を**改名**するか consumer を直す（**再コピーでは解決しない**） | 自作ステージ／プラグイン保有時 |
>
