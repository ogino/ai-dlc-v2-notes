...
> **⚠ 「2.6.2 まで reviewer を止められなかった」ではない。** 実行時オーバーライド `/aidlc --review none` は
> **2.5.54** から存在し、都度指定すれば止められた。2.6.18 で新しいのは
> **スコープの既定値として `none` を宣言したスコープが現れたこと**である
> （2.6.2 時点でスコープが宣言していた `review_cap` は `advisory` のみ）。
