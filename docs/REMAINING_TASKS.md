...
- [x] 上流 2.8.1 → 2.9.0 の差分反映（2026-09-17。`main` HEAD `2931ef02`）→ [18-release-impact-290.md](../18-release-impact-290.md)
- [x] 上流 2.9.0 → 2.10.0 の差分反映（2026-09-24。**タグ `v2.9.0` → タグ `v2.10.0`（`2a883858`）で測定**）→ [19-release-impact-2100.md](../19-release-impact-2100.md)
- [ ] **追従フローの定例化** — 担当者と頻度を決める
  ...
  - **⚠ ただし `aidlc update --check` は上流 `main` の追跡には使えない。**
    ...
    現に、18 章の基準 `2931ef02` の時点で **`main` は `v2.9.0` タグより 12 コミット先行しており**、
    2026-09-23 時点では 50 コミット以上先行していた（→ 18 章。`v2.10.0` の公開で解消）。
  ...
  - **一次のチェックは記録済み HEAD SHA と `origin/main` の比較である。**
    ...
    last=2a883858   # タグ v2.10.0（2026-09-24。同日の main は 20008a5d で +1、テストのみ）
...
- [ ] **🔴 リリース時のフルテストスイートが v2.10.0 で再び外れた（2026-09-24）** —
      `release.yml` のテストジョブは 0 本、`native-smoke` の `needs` は `[validate, verify]`。
      代わりに入った「フルスイート結果（evidence）の要求」もリリース準備コミット `2a883858` で外された。
      **再びリリース時に要求されるようになるかを追う**（→ [19.10](../19-release-impact-2100.md)）
- [x] **リリース時のフルテストスイートの無効化がいつ解除されるか（2026-09-17 解決 → 2026-09-24 に再び外れた。上の項目）** —
      `TEMPORARY` は HEAD から消えた。コメント解除ではなく**専用ジョブ 4 本への分割**で復活し、
      `native-smoke` が `test` に依存するようになった（→ [18.15](../18-release-impact-290.md)）
- [x] **v2.9.0 に残っていたセンサー・フック系の不具合 3 件を追跡する（2026-09-24 解決）** —
...
- [ ] **上流の「次の minor で削除」予告を追う**（2026-09-24 追加）—
      `## Review` 埋め込み形式（v2.8.1 予告、**v2.10.0 でも削除されず**）と、Change Control の旧名（v2.10.0 予告）。
      どちらもどの版で実際に消えるかを見る（→ [19.11](../19-release-impact-2100.md) / [19.3](../19-release-impact-2100.md)）
- [x] **上流 PR #1157（Kiro IDE と Kiro CLI の配布統合）を追跡する（2026-09-24 確認: マージされずに CLOSED）** —
      マージされると**ハーネスが 7 → 6 になりうる**としていた。`v2.10.0` でもハーネスは 7 種で、Kiro CLI と Kiro IDE は `.kiro/` を共有するため同じプロジェクトに置けない（→ [19.6](../19-release-impact-2100.md)）
...
- [x] **`audit-format.md` の宣言件数と表行数の不一致（2026-09-23 解決）** —
      ...
      **`261083ce`（#1150）で宣言 13 / 行 13 に是正され、`v2.10.0` に収録された**（`v2.9.0` は 11 / 10 のまま）。
...
> **📌 2026-09-24 時点の上流（本区間は追従済み）**
>
> | 項目 | 値 |
> |---|---|
> | Latest | **`v2.10.0`**（`2a883858`、2026-09-24 05:52 UTC 公開） |
> | `main` HEAD | `20008a5d`（タグから +1、テストのみ） |
> | 本区間（タグ `v2.9.0` → タグ `v2.10.0`） | **67 コミット / 620 ファイル / +117,900 −12,997** |
> | preview チャネル | 区間内は `v2.9.1-preview.20260915.1` / `.20260920.1` / `.20260921.1` の 3 本 |
> | 18 章の不具合 3 件 | **すべて `v2.10.0` に収録** |
>
> **この区間の追従は [19 章](../19-release-impact-2100.md) で完了した。**
