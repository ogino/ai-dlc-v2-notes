...
| `10-`〜`17-release-impact-*.md` | 版ごとの日付付き測定記録（本文は書き換えない） |

※ **2026-09-01 以前は `v2` ブランチだった。**`v2` は上流で削除され、`main` がその線形な継続である。
11〜17 章の `基準:` 行が記録している HEAD SHA は、いずれも `main` から到達できる（10 章は SHA を持たない）
（→ [16-release-impact-2700.md](../16-release-impact-2700.md)）。`v2_backup` というヘッドが残っているが、
**旧 `v2` HEAD のコピーではない**ので参照先にしないこと。

※※ **2026-09-08 に配布方式が変わった。** 上流から `dist/` が削除され、
正本の入口は「リポジトリの `dist/` を見る」から
「**タグ付きリリースの資産（provenance 署名付き）と `main` のソース**」に移った。
実装バージョンと最新タグは一致しないことがあるため、
**版を固定するときは `git ls-remote --tags origin` で実在を確認する**
