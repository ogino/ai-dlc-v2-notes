...
有効化には 3 段階の opt-in（サーバの `disabled` 解除 → ペルソナの `includeMcpJson` → `@server` grant）が要る。

設計上重要なのは、**`@server` がどのペルソナの `allowedTools` にも入っていない**ことである
（14 ペルソナすべてで確認され、テストが不変条件として固定している）。
つまりサーバを有効にしても、呼び出しのたびに承認が要る。指揮役（conductor）は MCP アクセスを一切持たない。
