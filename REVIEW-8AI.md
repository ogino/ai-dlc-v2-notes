...
- **原因別の内訳**（手元 **15 件** + GitHub 側 **24 件** = 採用 **39 件**）。
  この欄は 2 度書き直している。初版は「中身の誤りは 4 件で残りはすべてコピー同期漏れ」と書いて
  **過少分類**を指摘され、2 版目は合計を 36 件と数えて **3 件取りこぼし**を指摘された（いずれも Devin）。
  **集計の誤りを直す文章で、また集計を間違えている。**以下は表の行を機械的に数え直したものである。

  | 原因 | 件数 | 該当 |
  |---|---:|---|
  | **一次情報の確認漏れ・事実誤認** | **8** | #1 #3 #4 #5 G1 G3 G4 G5 |
  | **要約・代理値による事実の欠落** | **2** | I1（「同上」で圧縮）K1（判定基準を日付に置換） |
  | **集計・計算の誤り** | **2** | #2（76≠74）#15（「124 版分」） |
  | **同じ事実の別コピーの直し忘れ** | **9** | #12 G2 H1 H2 H3 H4 I2 I3 L1 |
  | 表現・体裁・編集ミス | 18 | 残り |

  合計 8+2+2+9+18 = **39**。

  **コピー同期漏れ（9 件）は最多ではない。** 同程度に多いのが
  **一次情報に当たらずに書いた／確認したつもりで断定した**類（8 件）で、
  さらに**集計そのものを 3 回続けて間違えている**（#2、#15、この欄）。
  対策をコピー同期だけに寄せると、残り 10 件超は次回も同じ形で出る。
  必要なのは 3 つで、それぞれ別の作業である ——
  **(1) 書く前に一次情報へ当たる**／**(2) 書いた後に全コピーを機械照合する**／
  **(3) 数を書いたら機械で数え直す**。

  **なお (2) にも落とし穴がある。** G2 の一括置換は「コード変更ゼロ」という**特定の文字列**を対象にしたため、
  同義の「コード変更は無い」を書いた 2 箇所を拾えず、M2 として再指摘された。
  **grep は書いた文字列しか見つけない。** 同期をとるときは表記ゆれの候補を先に列挙すること。

  **もう 1 つ、レビュー結果の取得自体を間違えていた。** GitHub API を `--paginate` 無しで叩いていたため、
  非公開側の指摘を 1 ページ目しか見ておらず、**10:45 以降の 3 件を「新規指摘なし」と誤報告**していた。
  レビューを取りこぼす読み方をしていては、上の 3 つを守っても意味がない。
