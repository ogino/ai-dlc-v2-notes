...
- **原因別の内訳**（手元 15 件 + GitHub 側 21 件 = 採用 36 件）。
  当初この欄に「中身の誤りは 4 件で残りはすべてコピー同期漏れ」と書いたが、
  **これは Devin の指摘どおり過少分類だった。** 実際は次のとおりである。

  | 原因 | 件数 | 例 |
  |---|---:|---|
  | **一次情報の確認漏れ・事実誤認** | **9** | `/aidlc plugin sync` の脱落、既存タグ（`v2.1.1` 等）の見落とし、ISSUE_TEMPLATE の由来、roadmap #754 の行方、「fetch に失敗する」の断定、2.7.0 への誤帰属 |
  | **要約による事実の欠落** | **2** | 「止まる 4 版」を「同上」で圧縮、判定基準を日付という代理値に置換 |
  | **集計・計算の誤り** | **2** | 変更内訳 76 ≠ 74、「124 版分」（実際は欠番があり 95 エントリ） |
  | **同じ事実の別コピーの直し忘れ** | **8** | 「コード変更ゼロ」11 ファイル、`reset --hard`、pin、404/302、警告バナー |
  | 表現・体裁・編集ミス | 15 | 時系列逆転、表崩れ、相対パス、章範囲の揺れ |

  **コピー同期漏れは最多ではない。** 最も多いのは**一次情報に当たらずに書いた／確認したつもりで断定した**類である。
  対策をコピー同期だけに寄せると、この 9 件は次回も再発する。
  **書く前に一次情報へ当たる**ことと、**書いた後に全コピーを機械照合する**ことは別の対策であり、両方要る。
