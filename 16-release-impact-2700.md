...
次回: [17-release-impact-2801.md](./17-release-impact-2801.md)（2.7.0 → 2.8.1、`main` HEAD `c03f9e28`）。
**本章本文の数値は 2.7.0 時点の測定記録として維持する。** 2.7.0 → 2.8.1 で動いたのは
`core/tools/*.ts` の 51 → 69 のみで、ワークフロー体系の数値はすべて不変である。
**ただし本章 16.1 が記録している導入手順（`git clone --branch main` して `dist/` を使う）は、
2.8.x で無効になった。** `dist/` は上流から削除され、導入はネイティブインストーラと
`aidlc config` に変わっている。
...
**⚠ 対象を「日付」で判定しないこと。** 2.6.1 の CHANGELOG 日付は 2026-08-13 だが、
**判定基準はワークフローを作った側のシェル（state スキーマ）が v8 より前かどうか**である。
2.6.1 の公開後も古いシェルを使い続けていれば、**2026-08-13 より後に作ったワークフローでも
pre-v8 state を持ち、拒否の対象になる**。日付で線を引くと、その分が漏れる。
