...
> **時点限定**: 上記「2.6.2 でも不変」のうち、スコープ 9・監査 82（21 分類）・CLI tools 38 は
> 2.6.49 でさらに動いている（スコープ **11**、監査 **86 種（22 分類）**、CLI tools **41**）。
> フェーズ 5・エージェント 14・フック 17・ハーネス 7 は 2.6.49 でも不変。
...
> - `1.103` は `Release date: August 7, 2025`（見出しは「July 2025」＝開発月）。**1 年近く前**にあたり、要件としては低すぎる
>
> **「月刊なら 1 年で 27 版は速すぎる」という反証**が出たため確認した。
> さらに「週次なら 1.103 は 2026 年初頭のはずで、2025 年ではない」という**再反証**も出たので、
> 4 点まで取った。
>
> **切り替わりは推定ではなく、上流が明言している。**
> [`updates/v1_111`](https://code.visualstudio.com/updates/v1_111) の冒頭:
>
> > Welcome to the 1.111 release of Visual Studio Code, **the first of our weekly Stable releases**!
>
> | 版 | `Release date`（逐語） | 見出し |
> |---|---|---|
> | 1.103 | `August 7, 2025` | 「July 2025 (version 1.103)」＝開発月を冠する旧形式 |
> | 1.110 | `March 4, 2026` | 「February 2026 (version 1.110)」＝旧形式の**最後** |
> | **1.111** | **`March 9, 2026`** | 「Visual Studio Code 1.111」＝**週次の初回**（上流の明記） |
> | 1.115 | `April 8, 2026` | 新形式 |
...
>
> **「平均間隔」であって各版の実間隔ではない**（中間版のリリース日は個別に確認していない）。
> ここで必要なのは各版の正確な間隔ではなく、**1.111 を境に運用が変わったこと**であり、
> それは上流が自ら書いている。**単一区間から外挿すると矛盾するのは、その区間が境をまたぐため**である。
>
> 2.5.60 のリリース（2026-08-09。上流 CHANGELOG の `## [2.5.60] - 2026-08-09`）は
> 1.130 の **18 日後**で、新規要件として整合する。
