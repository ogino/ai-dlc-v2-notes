...
| 項目 | 内容 |
|---|---|
| 必要バージョン | **上流記載**: Copilot CLI ≥ 1.0.74 / VS Code ≥ 1.130（`docs/guide/harnesses/copilot.md` と `dist/copilot/AGENTS.md` の両方が 1.130 と記載）。**当方では VS Code のリリース実在を確認していない**（[11.7](#117-本ノートの限界) 参照） |
...
- reviewer class の効果、`dist/copilot` の folder trust 挙動、PreToolUse deny の実効性は
  いずれも**上流の記述による**
- **VS Code の最低バージョン 1.130 は上流記載をそのまま引いている。**
  レビューで「2026-08 時点では未リリースの版番号ではないか（1.103 の転記ミスでは）」との指摘があったが、
  **当方に VS Code のリリース履歴を確認する手段がなく、上流の数字を勝手に書き換えることもしなかった**。
  導入前に実機で確認すること
