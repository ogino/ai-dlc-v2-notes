...
> 本章の数値は実装バージョン **2.7.0**（上流 `main` HEAD `96b11d39` / 取得日 2026-09-01）時点。
...
> 2.6.123 → 2.7.0 は [16-release-impact-2700.md](./16-release-impact-2700.md) を参照。
...
| **実装** | `awslabs/aidlc-workflows` の **`main` ブランチ**（GA。2026-09-01 に `v2` ブランチから移管） |

2.0 実装は方法論を **「検証可能で自己修正するエンジニアリング・ワークフロー」**として具体化し、次を実現する。

- **5 フェーズ / 33 ステージ**（原典 3 フェーズを実装都合で細分化・拡張）
- **14 エージェント・ロスタ**
- **11 アダプティブ・スコープ** + Composer
- **決定論的エンジン + LLM 導体**
...
| 決定理由が残らない | アーティファクト + **91 種**イベントの監査証跡 |
...
### 1.8.1 Bolt の定義は 2.6.86 で上流が書き換えた

**本ノートの旧記述が誤っていたのではなく、上流グロッサリの定義そのものが変わった。**
...
用語の位置づけが「実行単位」から「計画上のスライス」へ移った、という文書上の再定義である。
