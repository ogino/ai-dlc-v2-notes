...
未設定プロジェクトで TTY から引数なしで `aidlc config` を実行したときだけ、6 ステップのウィザードが動く。
...
| 2.8.0 | 2026-09-08 | `1c64d9a6`（#1046） | 2.7.x のロールアップ。2.7.2 から挙動変更なし | [17.3](#173--281-は-changelog-にあるがリリースされていない) |
...
- 2.8.1 の 2 件の修正は未リリースのため、**動作を確認する手段が現時点で無い**。
- 「2.8.1 調査で残った未確認事項」は
