...
次回: [18-release-impact-290.md](./18-release-impact-290.md)（2.8.1 → 2.9.0、`main` HEAD `2931ef02`）。
**本章本文の数値は 2.8.1 時点の測定記録として維持する。**
**ただし本章の基準 `c03f9e28` は、リリースされた `v2.8.1` タグ（＝ `215afe1a`）ではない。**
タグは 5 コミット後を指し、その差分には Change Control と Plan Approval の再束縛（#1000）が含まれる。
本章が「未リリース」「タグが無い」と書いている箇所は、いずれも 2026-09-09 時点の記録である（→ 18.1）。
...
> ## 📌 続報（2026-09-18 追記）—— 以下の囲みは v2.9.0 で前提が逆転した
>
> **v2.9.0 以降、手動コピー経路に必要なのは Bun であり、ネイティブ `aidlc` は不要である。**
> アセットも `aidlc-copy-runtime-X.Y.Z.tar.gz`（`dist/<harness>` 由来）に分離された。
> 上流 README 逐語（2.9.0）: `Install Bun … This path does not require the native aidlc command.`
> **→ [18.4](./18-release-impact-290.md) と [6 章の手動コピー節](./06-harnesses-install.md) を見ること。**
>
> **また、この囲みが提案しているソース生成による回避策はもう不要である。**
> Copilot / Cursor の不具合は `52da70ad` で修正され、**`v2.8.1` 以降に含まれる**
> （`git tag --contains 52da70ad` で確認済み）。
>
> 以下は **2026-09-09 時点（`c03f9e28`）の記録として保存する。手順としては採らないこと。**
...
> ## 📌 続報（2026-09-17 追記）—— 本章の警告は解消済み
>
> **本章は 2026-09-09 時点（`c03f9e28`）の測定記録である。その後、上流が大きく進んだ。**
>
> | 事実 | 内容 |
> |---|---|
...
> | **その後 v2.8.2（09-11）と v2.9.0（09-15）が公開** | **2026-09-17 時点の Latest は v2.9.0**（資産 15 件。**2026-09-24 に v2.10.0 が公開され、現 Latest は v2.10.0**） |
...
> **Copilot / Cursor を使うなら、v2.8.1 以降（推奨は現 Latest の v2.10.0）を導入すればよい。**
>
...
>
> 修正はコミット **`52da70ad`**（#1065）で入ったが、**それを含むリリースはまだ無い**。
> **公開時の版番号は未確定である** —— コミット本文は 2.8.2 と書き、CHANGELOG は 2.8.1 に統合している。
> **`v2.8.1` が出れば直る、とは限らない。** 修正の有無はタグ名ではなく `52da70ad` の包含で判定すること。
...
> **📌 以下は 2026-09-09 時点の指示である。現在は当てはまらない。**
> **`v2.8.1` / `v2.8.2` / `v2.9.0` はいずれも実在し、現在の固定先は `v2.10.0`（2026-09-24 以降の Latest）である**
> （→ [README](./README.md) / [18.1](./18-release-impact-290.md)）。
> **とくに `v2.8.0` は Copilot / Cursor のフックが動作しない唯一のリリースなので、今から選ぶ理由は無い。**
>
> 当時の指示（記録として保存）:
>
...
|---|---|---|---|---|
| 2.8.1 | 2026-09-08 | `c03f9e28`（#1054） | ウィザードの Enter 受理、umask 非依存の same-version update。**基準時点では 2 件。その後 `52da70ad` で Copilot / Cursor のフック修正が加わり 3 件になった**（17.3 の追記）。**未リリース** | [17.3](#173--281-は-changelog-にあるがリリースされていない) |
...
- 2.8.1 の修正は未リリースのため、**公開されているネイティブ資産では確認できない**。
  ソースから生成すれば検証自体は可能だが、**checkout する SHA は修正ごとに違う**
  （Enter 受理 / umask は `c03f9e28` 以降、**Copilot / Cursor のフック修正は `52da70ad` 以降**）。
  **本調査ではいずれも行っていない。**
- 「2.8.1 調査で残った未確認事項」は
