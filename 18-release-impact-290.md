...
コミット **50 件**、変更ファイル **473 件**（+57,789 −8,314 行）、期間 **10 日**（上流コミット日 2026-09-08 〜 09-17、両端を含む）。
...
**したがって Change Control（18.3）と Plan Approval の再束縛（18.7）は、
...
> 2.8.1 は「宣言 10 / 行 9」、2.9.0 は「宣言 11 / 行 10」。
> **既知の齟齬で、本区間でも解消していない**（申し送り事項として継続）。
...
**初出リリースは v2.9.0**（`a0ee4415` / #1160 ほか）。
