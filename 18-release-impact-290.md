...
> ## 📌 続報（2026-09-18 追記）—— 本章の結論は維持される
>
> 翌日に再取得したところ、`main` は **`87cc7f05`** まで 5 コミット進んでいた。
> **ただしリリースもタグも増えておらず、`AIDLC_VERSION` は 2.9.0 のまま。Latest も `v2.9.0`。**
>
> | 事実 | 内容 |
> |---|---|
> | 新規リリース | **無し**（`v2.9.0` が引き続き Latest） |
> | `core/tools/*.ts` | `main` 上で **71 → 72**（`aidlc-guard-operation.ts` が #1149 で追加。**未リリース**） |
> | ステージ / 監査イベント / バイパス / プロトコル | **すべて不変**（33 / 99 / 12 / 9） |
> | **#1070・#1166 の修正** | **依然として未リリース**。18.6 の警告はそのまま有効 |
>
> **本章の基準は `2931ef02` のままである**（`v2.9.0` タグ時点の `core/tools` も 71 で一致）。
...
**`main` では修正済みだが、タグの付いたリリースには入っていない。**
...
**漏れをコンパイル時エラーにする**が、**まだリリースされていない**。
...
実在する preview タグ: `v2.8.3-preview.20260914.1` / `v2.9.1-preview.20260915.1`。
