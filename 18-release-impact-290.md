...
次回: [19-release-impact-2100.md](./19-release-impact-2100.md)（2.9.0 → 2.10.0、**タグ `v2.9.0` → タグ `v2.10.0`**）
...
**本区間は中身が動いた区間である。** とくに**既定スコープ `classic` のステージ数が 26 → 18 に減った**（どの経路で `classic` に落ちるかは 18.2）。

---

> ## 📌 続報（2026-09-24 追記）—— `v2.10.0` がリリースされ、18.6 の 3 件はすべて解消した。18.15 は覆った
>
> | 事実 | 内容 |
> |---|---|
> | 新規の安定版 | **`v2.10.0`（`2a883858`、2026-09-24 05:52 UTC）。現 Latest** |
> | **18.6 の ①②③** | **すべて `v2.10.0` に収録**（`be94bde7` / `c97fa7ba` / `f79e321b`）。#1281 も収録 |
> | **18.15「テストが発行の前提条件に戻った」** | **覆った。** `v2.10.0` の `release.yml` にテストジョブは無く、`native-smoke` は `[validate, verify]` だけに依存する（→ 19.10） |
> | **18.11「次の minor で `## Review` 埋め込み形式を廃止」** | **`v2.10.0`（minor）では廃止されず、予告ごと持ち越された**（→ 19.11） |
> | 18.3 の Change Control | **Guard Policy に改名**され、`relaxed` の意味が広がった（→ 19.3） |
>
> **本章は 2.8.1 → 2.9.0 の記録として凍結する。** 以降は [19 章](./19-release-impact-2100.md) を参照。
> 19 章は 18.1 の教訓から**タグからタグで**測っており、`v2.9.0` タグから本章の終点 `2931ef02` までの 12 コミットも 19 章の区間に含まれる。

> ## 📌 続報（2026-09-23 追記）—— 本章の結論は維持される。①② には preview 経路ができたが、③ #1249 が新たに開示された
>
> | 事実 | 内容 |
> |---|---|
> | `main` の到達点 | **`b4793e58`**（2026-09-23 マージ直前に再取得）。本章の終点から **50 コミット** |
...
**ただし「スコープ名を指定しない全利用者」ではない。** 対話的に `/aidlc <説明文>` を打ってキーワードに当たらなければ、上流は先に compose 提案を出し、黙って既定スコープを始めない（→ [5.2.1](./05-scopes-depth-test.md)）。**`classic` に落ちるのは、compose 提案を経由しないフォールバック経路**（`/aidlc-init`、`--scope` 無しの `intent-create`、既定スコープの解決）と、**`classic` を明示的に選んだ場合**である。
...
## 18.6 🔴 現在の Latest（v2.9.0）に残っているセンサー・フック系の不具合 3 件

> **📌 2026-09-24: 3 件とも `v2.10.0` に収録された（→ [19.4](./19-release-impact-2100.md)）。以下は v2.9.0 に留まる場合の記録である。**

**①② はネイティブ（コンパイル済みバイナリ）インストール限定で、`bun` 実行では再現しない。**
**①② は `main` で修正済みで、安定版（Latest `v2.9.0`）には未収録、preview `v2.9.1-preview.20260920.1` 以降には収録済み**（2026-09-23 実測）。
**③（#1249、2026-09-23 追記）は性質が違う** —— `bun` 実行でもフックの PATH 上に `bun` が無ければ起こりうるうえ、
**修正はどの版にも未収録（preview を含む）**である（→ 下記 ③）。
...
### ✅ 今日使える回避策 —— 手動コピー経路なら ①② は踏まない（③ は条件付き）
...
①② は踏まない。** これは上流が公認する正規経路である（→ 18.4）。**③ は下記のとおり条件付きである。**
...
> **📌 2026-09-24: 次の minor である `v2.10.0` では廃止されず、予告ごと持ち越された（→ [19.11](./19-release-impact-2100.md)）。**
...
> **📌 2026-09-24: `v2.10.0` で再び外れた**（テストジョブ 0 本、`needs: [validate, verify]`。→ [19.10](./19-release-impact-2100.md)）。
...
- **上流のテストスイートは実行していない**（調査環境に `bun` が無い）。
  18.6 の 3 件は**コードの読解と版間 diff による判定**（#1249 は上流の修正コミット本文にある実測）であり、当方が実機で再現させたものではない
- **ネイティブインストーラを実機で走らせていない。** 18.4 のアセット名はリリース資産一覧の実測である。
  **手動コピー用の `aidlc-copy-runtime-2.9.0.tar.gz` は実物を展開して中身を確認し、claude ハーネスについては既存プロジェクトへの適用手順を使い捨てのプロジェクトで流した**（→ 6 章の手動コピー節）。**他ハーネスへの適用と、ネイティブ用 `aidlc-runtime-*.tar.gz` の中身は確認していない**
