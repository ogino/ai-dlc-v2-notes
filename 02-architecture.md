...
**tools 本数（2.6.2 実測）**: `core/tools/aidlc-*.ts` は **37 本**、ディスパッチャ `aidlc.ts` を含む `.ts` 全体では **38 本**（`aidlc.ts` はディスパッチャ本体で `aidlc-*` パターンに含まれない）。2.5.62 時点は 36 本＋ディスパッチャで計 37 だった（2.6.2 で `aidlc-sensor-traceability.ts` が加わった）。一方、**公式 README のツリー図は `25 aidlc-*.ts engine tools` と表記しており（`README.md:365`）、実数と 12 本ずれている**。
