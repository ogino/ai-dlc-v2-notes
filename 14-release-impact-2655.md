...
> **むしろ受理される文字列空間は狭く・完全に既知になった。**
> 旧来は「キャンセル定型文でなく、自己申告文言でもない任意の散文」が必要だったが、
> 今は `Approve` という 1 リテラルで足りる。**捏造の難易度は下がりこそすれ上がっていない。**
...
> （**ACP はハーネスではなくトランスポートである。** 上流のテストは Kiro CLI を
> ACP・TUI・IDE の複数経路で回しており、ACP 経路だけが ndjson トレースを残す。
> ハーネスは 7 種のままで、ACP はその 1 つである Kiro CLI の接続方式の 1 つ。）
